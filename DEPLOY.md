...
- **Vercel** (다음 이름 그대로 사용 가능):
  - `SUPABASE_URL` = `https://oljekyqhsqawqmxzpkeg.supabase.co`
  - `SUPABASE_KEY` = Supabase publishable / anon 키 (`sb_publishable_...`)
  - `API_URL` = `https://markettinghowa-5.onrender.com` (선택; 없으면 코드 기본 Render URL 사용)
  - `next.config.mjs`가 빌드 시 `SUPABASE_*` → `NEXT_PUBLIC_SUPABASE_*` 로 넘겨 브라우저에서 사용 가능하게 함.
- **Render**: `SUPABASE_URL`, `SUPABASE_KEY` 동일 이름 사용 가능. 서버 권한이 필요하면 `SUPABASE_KEY`에 **service_role** 키 권장 (publishable만으로는 RLS에 막힐 수 있음).
- 예전 이름(`NEXT_PUBLIC_SUPABASE_URL` 등)도 그대로 동작합니다.
- 전체 목록은 **`.env.example`** 참고. 백엔드는 `app/core/config.py`의 Settings에서 읽습니다.
...
| 변수명 | 설명 | 배포 시 값 |
|--------|------|------------|
| `SUPABASE_URL` 또는 `NEXT_PUBLIC_SUPABASE_URL` | Supabase URL | `https://oljekyqhsqawqmxzpkeg.supabase.co` |
| `SUPABASE_KEY` 또는 `NEXT_PUBLIC_SUPABASE_ANON_KEY` | publishable / anon 키 | `sb_publishable_...` |
| `API_URL` 또는 `NEXT_PUBLIC_API_URL` | 백엔드 API (선택) | `https://markettinghowa-5.onrender.com` |
...
| `SUPABASE_KEY` 또는 `SUPABASE_SERVICE_ROLE_KEY` | 서버용 키 (service_role 권장) |
...
4. **Environment**에 `SUPABASE_URL`, `SUPABASE_KEY`(또는 `SUPABASE_SERVICE_ROLE_KEY`), `GEMINI_API_KEY` 등 추가.
...
3. **Environment Variables**에 다음 중 하나 방식으로 추가:
   - **권장(현재 팀 설정)**: `SUPABASE_URL`, `SUPABASE_KEY`, (선택) `API_URL` = `https://markettinghowa-5.onrender.com`
   - 또는: `NEXT_PUBLIC_SUPABASE_URL`, `NEXT_PUBLIC_SUPABASE_ANON_KEY`, `NEXT_PUBLIC_API_URL`
4. **Deploy** 실행 (변경 후 Redeploy).
